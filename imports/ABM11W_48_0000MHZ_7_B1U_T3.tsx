...
export const ABM11W_48_0000MHZ_7_B1U_T3 = (props: ChipProps<typeof pinLabels>) => {
  return (
    <chip
      ...
      // C1985532 EasyEDA symbol, reimported with tsci import --use-exact-footprint.
      // Keep ground aliases unique and use explicit schematic stroke widths.
      symbol={
        <symbol>
          <schematictext text={props.name ?? "Y"} schX={0} schY={0.7} anchor="center" fontSize={0.18} color="#006464" />
          <schematictext text="48MHz / 7pF" schX={0} schY={-0.65} anchor="center" fontSize={0.18} color="#006464" />
          ...
          <schematicrect schX={0} schY={0} width={0.8} height={0.8} color="#880000" strokeWidth={0.02} />
          <schematiccircle center={{ x: -0.3, y: -0.3 }} radius={0.03} color="#880000" isFilled fillColor="#880000" strokeWidth={0.02} />
          <schematicpath points={[{ x: -0.1, y: -0.14 }, { x: -0.1, y: 0.14 }]} strokeColor="#881100" strokeWidth={0.02} />
          <schematicpath points={[{ x: 0.1, y: -0.14 }, { x: 0.1, y: 0.14 }]} strokeColor="#881100" strokeWidth={0.02} />
          <schematicpath points={[{ x: -0.4, y: -0.2 }, { x: -0.2, y: -0.2 }, { x: -0.2, y: 0 }, { x: -0.1, y: 0 }]} strokeColor="#880000" strokeWidth={0.02} />
          <schematicpath points={[{ x: 0.4, y: 0.2 }, { x: 0.2, y: 0.2 }, { x: 0.2, y: 0 }, { x: 0.1, y: 0 }]} strokeColor="#880000" strokeWidth={0.02} />
          <schematicrect schX={0} schY={0} width={0.08} height={0.28} color="#880000" strokeWidth={0.02} />
        </symbol>
      }
      supplierPartNumbers={{
  "jlcpcb": [
