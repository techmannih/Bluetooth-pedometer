...
check, routed build and default all-layer Gerber shorts check in order. Each
step must succeed before the next runs. To repeat the shorts check alone, use
`bunx tsci check shorts dist/index/circuit.json`.

No custom `scripts/` directory is required. The former `audit`, `test` and
...
unrouted artifact: run the subsequent full `build` before checking shorts or
using fabrication outputs. Snapshot discovery is restricted to `index.circuit.tsx`
...
option to **1200000 ms (20 minutes)** so the dense-board DRC-improvement
stage can finish beyond the default 10-minute worker limit. This is a bounded
runtime allowance, not a faster router or a DRC bypass. Routing effort,
clearances, dependencies and PCB geometry are unchanged.
...
exit alone as verification: inspect the circuit error records and run the
separate Gerber shorts check. See `VALIDATION.md` for measured local results;
