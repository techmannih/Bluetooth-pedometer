...
function getSvgFromGraphicsObject(graphics, {
  ...
  const svgObject = {
    ...
    children: [
      ...
      // Lines
      ...(graphics.lines || []).map((line, originalIndex) => ({ line, originalIndex })).sort(
        (a, b) => (a.line.zIndex ?? 0) - (b.line.zIndex ?? 0) || a.originalIndex - b.originalIndex
      ).map(({ line }) => {
        const projectedPoints = line.points.map(
          (p) => projectPoint(p, matrix)
        );
        return {
          name: "g",
          type: "element",
          ...
          children: [
            {
              ...
              attributes: {
                ...
                ...rect.ccwRotationDegrees !== void 0 && {
                  "data-ccw-rotation-degrees": rect.ccwRotationDegrees.toString()
                },
                x: rectX.toString(),
                y: rectY.toString(),
            ...
            ...shouldRenderLabel("rects") && rect.label ? [
              {
                ...
                attributes: {
                  ...
                  "font-size": ((projectedRect.width + projectedRect.height) / 2 * 0.06).toString(),
      ...
      ...(graphics.polygons || []).map((polygon) => {
        const projectedPoints = polygon.points.map(
          (point) => projectPoint(point, matrix)
        );
        const xs = projectedPoints.map((p) => p.x);
        const ys = projectedPoints.map((p) => p.y);
        const minX = xs.length > 0 ? Math.min(...xs) : 0;
        const minY = ys.length > 0 ? Math.min(...ys) : 0;
        const polygonStrokeWidth = polygon.strokeWidth === void 0 ? Math.abs(1 / matrix.a) : strokeScale * polygon.strokeWidth;
        return {
          name: "g",
          type: "element",
          attributes: {},
          children: [
            {
              ...
              attributes: {
                ...
                "data-points": polygon.points.map((p) => `${p.x},${p.y}`).join(" "),
                points: projectedPoints.map((p) => `${p.x},${p.y}`).join(" "),
...
//# sourceMappingURL=chunk-WDTSQY35.js.map