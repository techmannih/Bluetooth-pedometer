import {
  strokeAlphabetText
} from "./chunk-JW2BLZEM.js";
import {
  createSoftwareRasterSurface
} from "./chunk-B4JXZUEC.js";
import {
...
// lib/getPngBufferFromGraphicsObject.ts
import { applyToPoint } from "transformation-matrix";
var DEFAULT_PNG_SIZE = 640;
var DEFAULT_BACKGROUND_COLOR = "white";
var hasVisiblePaint = (value) => {
  return Boolean(value && value !== "none" && value !== "transparent");
};
var shouldRenderLabel = (includeTextLabels, disableLabels, type) => {
...
var parseStrokeDash = (strokeDash, strokeScale) => {
  if (typeof strokeDash === "string") {
    if (strokeDash.includes(",")) {
      return strokeDash.split(",").map((part) => Number.parseFloat(part.trim())).filter((value) => Number.isFinite(value) && value > 0);
    }
    const singleValue = Number.parseFloat(strokeDash.trim());
    return Number.isFinite(singleValue) && singleValue > 0 ? [singleValue] : [];
  }
  return strokeDash.map((segment) => segment * strokeScale).filter((value) => Number.isFinite(value) && value > 0);
};
var normalizeDashPattern = (dashPattern) => {
  if (dashPattern.length === 0) return dashPattern;
  return dashPattern.length % 2 === 0 ? dashPattern : [...dashPattern, ...dashPattern];
};
var strokePolyline = (ctx, points, dashPattern) => {
  ...
  for (let i = 1; i < points.length; i++) {
    ...
    while (distanceAlongSegment < segmentLength) {
      const remainingPatternLength = normalizedPattern[patternIndex] - patternOffset;
...
async function getPngBufferFromGraphicsObject(graphics, {
  ...
  const yFlip = transformOptions.yFlip ?? graphics.coordinateSystem === "cartesian";
  const matrix = transformOptions.transform ?? computeTransformFromViewbox(
  ...
  for (const line of (graphics.lines || []).map((value, originalIndex) => ({ value, originalIndex })).sort(
    (a, b) => (a.value.zIndex ?? 0) - (b.value.zIndex ?? 0) || a.originalIndex - b.originalIndex
  ).map(({ value }) => value)) {
    const projectedPoints = line.points.map(
      (point) => applyToPoint(matrix, point)
    );
    const dashPattern = line.strokeDash ? parseStrokeDash(line.strokeDash, strokeScale) : void 0;
    ctx.strokeStyle = line.strokeColor || "black";
    ctx.lineWidth = line.strokeWidth === void 0 ? 1 : Math.max(line.strokeWidth * strokeScale, 1);
    strokePolyline(ctx, projectedPoints, dashPattern);
    if (shouldRenderLabel(
      includeTextLabels,
      transformOptions.disableLabels,
      "lines"
    ) && line.label && projectedPoints.length > 0) {
      renderText(ctx, {
        text: line.label,
  ...
  if (graphics.infiniteLines) {
    ...
    for (const line of graphics.infiniteLines) {
      ...
      if (!segment) continue;
      const projectedPoints = segment.map(
        (point) => applyToPoint(matrix, point)
      );
      const dashPattern = line.strokeDash ? parseStrokeDash(line.strokeDash, strokeScale) : void 0;
      ctx.strokeStyle = line.strokeColor || "black";
  ...
  for (const polygon of graphics.polygons || []) {
    ...
    const projectedPoints = polygon.points.map(
      (point) => applyToPoint(matrix, point)
    );
    ...
    if (hasVisiblePaint(polygon.stroke)) {
      ...
      ctx.lineWidth = polygon.strokeWidth === void 0 ? 1 : Math.max(polygon.strokeWidth * strokeScale, 1);
      ctx.stroke();
    }
    if (shouldRenderLabel(
      includeTextLabels,
  ...
  for (const arrow of graphics.arrows || []) {
    ...
    if (shaftStart.x !== shaftEnd.x || shaftStart.y !== shaftEnd.y) {
      ...
      strokePolyline(ctx, [shaftStart, shaftEnd], void 0);
    }
    ctx.fillStyle = color;
    ...
    if (!transformOptions.disableLabels && !hideInlineLabels && arrow.inlineLabel) {
      renderText(ctx, {
...
//# sourceMappingURL=chunk-MITCOGY2.js.map