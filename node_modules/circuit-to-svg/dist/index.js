...
var alphabetBounds = (() => {
  ...
  let minY = Infinity;
  let maxY = -Infinity;
  for (const segments of Object.values(lineAlphabet2)) {
...
function createSvgObjectsFromPcbCopperText(pcbCopperText, ctx) {
  ...
  if (!text) return [];
  const [ax, ay] = applyToPoint20(transform, [
    anchor_position.x,
    anchor_position.y
  ]);
...
function createSvgObjectsFromPcbSilkscreenLine(pcbSilkscreenLine, ctx) {
  ...
  const [transformedX1, transformedY1] = applyToPoint22(transform, [x1, y1]);
  const [transformedX2, transformedY2] = applyToPoint22(transform, [x2, y2]);
  const transformedStrokeWidth = stroke_width * Math.abs(transform.a);
  const color = layer === "bottom" ? colorMap2.silkscreen.bottom : colorMap2.silkscreen.top;
  return [
    {
...
function createSvgObjectsFromPcbCourtyardCircle(pcbCourtyardCircle, ctx) {
  ...
  const [transformedX, transformedY] = applyToPoint28(transform, [
    center.x,
    center.y
  ]);
  const transformedRadius = radius * Math.abs(transform.a);
...
function getCopperPourShape(pour, transform) {
  ...
  if (pour.shape === "polygon") {
    ...
    const pointsString = pour.points.map((p) => applyToPoint30(transform, [p.x, p.y])).map((p) => `${p[0]},${p[1]}`).join(" ");
    return {
      elementType: "polygon",
...
function createSvgObjectsFromPcbTrace(trace, ctx) {
  ...
  for (const originalSegment of getPcbTraceSegments(trace.route)) {
    ...
    const startPoint = applyToPoint31(transform, [
      segment.start.x,
      segment.start.y
    ]);
    const endPoint = applyToPoint31(transform, [segment.end.x, segment.end.y]);
    ...
    const pourMaskAttributes = pourMaskId ? { mask: `url(#${pourMaskId})` } : {};
    if (pourMaskId) {
...
function createAnchorOffsetIndicators(params) {
  ...
  const objects = [];
  const [screenGroupAnchorX, screenGroupAnchorY] = applyToPoint33(transform, [
    groupAnchorPosition.x,
    groupAnchorPosition.y
  ]);
  const [screenComponentX, screenComponentY] = applyToPoint33(transform, [
    componentPosition.x,
    componentPosition.y
  ]);
...
function createSvgObjectsForRatsNest(circuitJson, ctx) {
  ...
  for (const line of ratsNestLines) {
    const transformedStart = applyToPoint37(transform, [
      line.startPoint.x,
      line.startPoint.y
    ]);
    const transformedEnd = applyToPoint37(transform, [
      line.endPoint.x,
      line.endPoint.y
    ]);
    const attributes = {
...
function createSvgObjectsFromPcbCutout(cutout, ctx) {
  ...
  if (cutout.shape === "polygon") {
    ...
    const transformedPoints = polygonCutout.points.map(
      (p) => applyToPoint38(transform, [p.x, p.y])
    );
    const pointsString = transformedPoints.map((p) => `${p[0]},${p[1]}`).join(" ");
    return [
      {
...
function createSvgObjectsFromPcbKeepout(keepout, ctx) {
  ...
  for (const layer of keepout.layers) {
    ...
    if (keepout.shape === "rect") {
      ...
      const [cx, cy] = applyToPoint40(transform, [
...
function createSvgObjectsFromSolderPaste(solderPaste, ctx) {
  ...
  const [x, y] = applyToPoint41(transform, [solderPaste.x, solderPaste.y]);
...
function createSvgObjectsFromPcbGroup(pcbGroup, ctx) {
  ...
  const halfWidth = width / 2;
  const halfHeight = height / 2;
  const [topLeftX, topLeftY] = applyToPoint43(transform, [
    center.x - halfWidth,
    center.y + halfHeight
  ]);
  const [bottomRightX, bottomRightY] = applyToPoint43(transform, [
    center.x + halfWidth,
    center.y - halfHeight
  ]);
  const rectX = Math.min(topLeftX, bottomRightX);
...
function createSvgObjectsFromAssemblyComponent(params, ctx) {
  ...
  const [pinX, pinY] = applyToPoint47(transform, [portPosition.x, portPosition.y]);
  const scaledWidth = width * Math.abs(transform.a);
  const scaledHeight = height * Math.abs(transform.d);
...
function createSvgObjectsFromAssemblyPlatedHole(hole, ctx) {
  ...
  if (hole.shape === "circular_hole_with_rect_pad") {
    ...
    const holeRadius = scaledHoleDiameter / 2;
    const [holeCx, holeCy] = applyToPoint49(transform, [
  ...
  if (hole.shape === "pill_hole_with_rect_pad") {
    ...
    const holeOffsetX = pillHoleWithOffsets.hole_offset_x ?? 0;
    const holeOffsetY = pillHoleWithOffsets.hole_offset_y ?? 0;
    const [holeCenterX, holeCenterY] = applyToPoint49(transform, [
      pillHole.x + holeOffsetX,
      pillHole.y + holeOffsetY
    ]);
    const holeRadius = Math.min(scaledHoleHeight, scaledHoleWidth) / 2;
...
function createSvgObjectFromAssemblyBoundary(transform, minX, minY, maxX, maxY) {
  const [x1, y1] = applyToPoint51(transform, [minX, minY]);
  const [x2, y2] = applyToPoint51(transform, [maxX, maxY]);
  const width = Math.abs(x2 - x1);
  const height = Math.abs(y2 - y1);
  const x = Math.min(x1, x2);
  const y = Math.min(y1, y2);
...
function createSvgObjectsFromPinoutBoard(pcbBoard, ctx) {
  ...
  if (outline && Array.isArray(outline) && outline.length >= 3) {
    path = outline.map((point, index) => {
      const [x, y] = applyToPoint52(transform, [point.x, point.y]);
      return index === 0 ? `M ${x} ${y}` : `L ${x} ${y}`;
    }).join(" ");
  } else {
    const halfWidth = width / 2;
    ...
    const topRight = applyToPoint52(transform, [
      center.x + halfWidth,
      center.y - halfHeight
    ]);
    ...
    const bottomLeft = applyToPoint52(transform, [
      center.x - halfWidth,
      center.y + halfHeight
    ]);
    path = `M ${topLeft[0]} ${topLeft[1]} L ${topRight[0]} ${topRight[1]} L ${bottomRight[0]} ${bottomRight[1]} L ${bottomLeft[0]} ${bottomLeft[1]}`;
  ...
  for (const cutout of cutlery) {
    if (cutout.shape === "rect") {
      ...
      const [tl, tr, br, bl] = [
        applyToPoint52(transform, [x - halfWidth, y - halfHeight]),
        applyToPoint52(transform, [x + halfWidth, y - halfHeight]),
        applyToPoint52(transform, [x + halfWidth, y + halfHeight]),
        applyToPoint52(transform, [x - halfWidth, y + halfHeight])
      ];
...
function createSvgObjectsFromPinoutComponent(elm, ctx) {
  ...
  const [x, y] = applyToPoint53(transform, [center.x, center.y]);
  const scaledWidth = width * Math.abs(transform.a);
  const scaledHeight = height * Math.abs(transform.d);
...
function createSvgObjectsFromPinoutPlatedHole(hole, ctx) {
  const { transform } = ctx;
  const [x, y] = applyToPoint55(transform, [hole.x, hole.y]);
  if (hole.shape === "pill") {
    const scaledOuterWidth = hole.outer_width * Math.abs(transform.a);
    const scaledOuterHeight = hole.outer_height * Math.abs(transform.a);
    const scaledHoleWidth = hole.hole_width * Math.abs(transform.a);
...
function createSvgObjectsFromPinoutSmtPad(pad, ctx) {
  ...
  if (pad.shape === "rect" || pad.shape === "rotated_rect") {
    const width = pad.width * Math.abs(transform.a);
    const height = pad.height * Math.abs(transform.d);
    const [x, y] = applyToPoint56(transform, [pad.x, pad.y]);
    if (pad.shape === "rotated_rect" && pad.ccw_rotation) {
      return [
        {
          name: "rect",
  ...
  if (pad.shape === "polygon") {
    const points = (pad.points ?? []).map(
      (point) => applyToPoint56(transform, [point.x, point.y])
    );
    return [
      {
        name: "polygon",
...
function createSvgObjectsFromPinoutPort(pcb_port, ctx) {
  ...
  const [port_x, port_y] = applyToPoint57(ctx.transform, [pcb_port.x, pcb_port.y]);
...
function calculateVerticalEdgeLabels(edge, pinout_labels, {
  ...
  const board_edge_x = applyToPoint58(transform, [
...
function drawSchematicGrid(params) {
  ...
  const transformPoint = (x, y) => {
    const [transformedX, transformedY] = applyToPoint59(params.transform, [x, y]);
...
function drawSchematicLabeledPoints(params) {
  ...
  for (const point of points) {
    const [x1, y1] = applyToPoint60(transform, [point.x - 0.1, point.y - 0.1]);
    const [x2, y2] = applyToPoint60(transform, [point.x + 0.1, point.y + 0.1]);
    const [x3, y3] = applyToPoint60(transform, [point.x - 0.1, point.y + 0.1]);
    const [x4, y4] = applyToPoint60(transform, [point.x + 0.1, point.y - 0.1]);
    ...
    const [labelX, labelY] = applyToPoint60(transform, [
      point.x + 0.15,
      point.y - 0.15
    ]);
    labeledPointsGroup.push({
...
function getComponentSymbolTextBounds({
  ...
  for (const primitive of symbol.primitives) {
    ...
    if (!text) continue;
    const anchorPosition = applyToPoint61(symbolToRealTransform, primitive);
...
var createSvgSchErrorText = ({
  ...
  const screenCenter = applyToPoint62(realToScreenTransform, realCenter);
  return {
    type: "element",
    name: "text",
    value: "",
...
var createSvgObjectsForSchPortIndicator = ({
  ...
  const screenPos = applyToPoint66(transform, schPort.center);
  svgObjects.push({
    name: "circle",
    type: "element",
    value: "",
  ...
  if (label) {
    ...
    const screenLabelPos = applyToPoint66(transform, labelPos);
...
var createSvgSchText = ({
  ...
  const center = applyToPoint67(transform, elm.position);
  const textAnchorMap = {
    center: "middle",
    center_right: "end",
    bottom_left: "start",
...
var createSvgObjectsFromSchematicBox = ({
  ...
  const topLeft = applyToPoint69(transform, {
    x: schematicBox.x,
    y: schematicBox.y
  });
  const bottomRight = applyToPoint69(transform, {
...
var createSvgObjectsFromSchematicComponentWithSymbol = ({
  ...
  const [screenMinX, screenMinY] = applyToPoint71(
    compose13(realToScreenTransform, transformFromSymbolToReal),
    [bounds.minX, bounds.minY]
  );
  const [screenMaxX, screenMaxY] = applyToPoint71(
    compose13(realToScreenTransform, transformFromSymbolToReal),
    [bounds.maxX, bounds.maxY]
  );
  const rectHeight = Math.abs(screenMaxY - screenMinY);
  ...
  for (const text of texts) {
    const screenTextPos = applyToPoint71(
      compose13(realToScreenTransform, transformFromSymbolToReal),
      text
    );
    let textValue = "";
  ...
  for (const box of boxes) {
    const screenBoxPos = applyToPoint71(
      compose13(realToScreenTransform, transformFromSymbolToReal),
      box
    );
  ...
  for (const port of symbol.ports) {
    ...
    const screenPortPos = applyToPoint71(
      compose13(realToScreenTransform, transformFromSymbolToReal),
      port
    );
    svgObjects.push({
  ...
  for (const circle of circles) {
    const screenCirclePos = applyToPoint71(
      compose13(realToScreenTransform, transformFromSymbolToReal),
      circle
    );
...
var createSvgObjectsForSchPortBoxLine = ({
  ...
  const screenSchPortPos = applyToPoint72(transform, schPort.center);
  const screenRealEdgePos = applyToPoint72(transform, realEdgePos);
  ...
  const screenLineEnd = applyToPoint72(transform, realLineEnd);
...
var createSvgObjectsForSchPortPinNumberText = (params) => {
  ...
  const screenPinNumberTextPos = applyToPoint73(transform, realPinNumberPos);
  svgObjects.push({
    name: "text",
    type: "element",
    attributes: {
...
var createSvgObjectsForSchPortPinLabel = (params) => {
  ...
  const screenPinNumberTextPos = applyToPoint74(transform, realPinNumberPos);
...
var createSvgObjectsFromSchematicComponentWithBox = ({
  ...
  const svgObjects = [];
  const componentScreenTopLeft = applyToPoint76(transform, {
    x: schComponent.center.x - schComponent.size.width / 2,
    y: schComponent.center.y + schComponent.size.height / 2
  });
  const componentScreenBottomRight = applyToPoint76(transform, {
    x: schComponent.center.x + schComponent.size.width / 2,
    y: schComponent.center.y - schComponent.size.height / 2
  });
...
function createSvgObjectsFromSchematicLine({
  ...
  const p1 = applyToPoint77(transform, { x: schLine.x1, y: schLine.y1 });
  const p2 = applyToPoint77(transform, { x: schLine.x2, y: schLine.y2 });
...
function createSvgObjectsFromSchematicRect({
  ...
  const center = applyToPoint78(transform, schRect.center);
...
function createSvgObjectsFromSchematicPath({
  ...
  const transformedPoints = schPath.points.map(
    (p) => applyToPoint79(transform, { x: p.x, y: p.y })
  );
...
function createSvgObjectsFromSchDebugObject({
  ...
  if (debugObject.shape === "rect") {
    let [screenLeft, screenTop] = applyToPoint80(transform, [
      debugObject.center.x - debugObject.size.width / 2,
      debugObject.center.y - debugObject.size.height / 2
    ]);
    let [screenRight, screenBottom] = applyToPoint80(transform, [
      debugObject.center.x + debugObject.size.width / 2,
      debugObject.center.y + debugObject.size.height / 2
    ]);
    ...
    const [screenCenterX, screenCenterY] = applyToPoint80(transform, [
  ...
  if (debugObject.shape === "line") {
    const [screenStartX, screenStartY] = applyToPoint80(transform, [
      debugObject.start.x,
      debugObject.start.y
    ]);
    const [screenEndX, screenEndY] = applyToPoint80(transform, [
      debugObject.end.x,
      debugObject.end.y
    ]);
...
function createSheetRect({
  ...
  const screenCenter = applyToPoint81(transform, center);
...
function createSheetLine({
  ...
  const p1 = applyToPoint81(transform, { x: x1, y: y1 });
  const p2 = applyToPoint81(transform, { x: x2, y: y2 });
  return {
    name: "line",
    type: "element",
    value: "",
...
function createSheetText({
  ...
  const center = applyToPoint81(transform, { x, y });
  return {
    type: "element",
    name: "text",
    value: "",
...
var createSvgObjectsFromSchematicTable = ({
  ...
  const [screenTopLeftX, screenTopLeftY] = applyToPoint82(transform, [
    topLeftX,
    topLeftY
  ]);
  const [screenBottomRightX, screenBottomRightY] = applyToPoint82(transform, [
    topLeftX + totalWidth,
    topLeftY - totalHeight
  ]);
  svgObjects.push({
  ...
  for (let i = 0; i < column_widths.length - 1; i++) {
    ...
    for (let j = 0; j < row_heights.length; j++) {
      ...
      if (!isMerged) {
        const start = applyToPoint82(transform, { x: currentX, y: segmentStartY });
        const end = applyToPoint82(transform, { x: currentX, y: segmentEndY });
        svgObjects.push({
          name: "line",
          type: "element",
          attributes: {
  ...
  for (let i = 0; i < row_heights.length - 1; i++) {
    ...
    for (let j = 0; j < column_widths.length; j++) {
      ...
      if (!isMerged) {
        const start = applyToPoint82(transform, {
          x: segmentStartX,
          y: currentY
        });
        const end = applyToPoint82(transform, { x: segmentEndX, y: currentY });
        svgObjects.push({
          name: "line",
          type: "element",
          attributes: {
  ...
  for (const cell of cells) {
    if (cell.text) {
      ...
      const screenTextAnchorPos = applyToPoint82(transform, realTextAnchorPos);
...
function createSchematicTrace({
  ...
  for (let edgeIndex = 0; edgeIndex < edges.length; edgeIndex++) {
    ...
    const [screenFromX, screenFromY] = applyToPoint83(transform, [
      edge.from.x,
      edge.from.y
    ]);
    const [screenToX, screenToY] = applyToPoint83(transform, [
      edge.to.x,
      edge.to.y
    ]);
  ...
  if (trace.junctions) {
    for (const junction of trace.junctions) {
      const [screenX, screenY] = applyToPoint83(transform, [
        junction.x,
        junction.y
      ]);
      overlayObjects.push({
...
function createSvgObjectsFromSchVoltageProbe({
  ...
  const [screenX, screenY] = applyToPoint84(transform, [
    probe.position.x,
    probe.position.y
  ]);
...
function convertCircuitJsonToSchematicSvg(circuitJson, options) {
  ...
  const sheetCircuitJson = selectedSheet ? circuitJson.filter(
    (elm) => !elm.type.startsWith("schematic_") || "schematic_sheet_id" in elm && elm.schematic_sheet_id === selectedSheet.schematic_sheet_id
  ) : circuitJson;
...
import { parseSync as parseSync3, stringify as stringify5 } from "svgson";
import { applyToPoint as applyToPoint86, fromString } from "transformation-matrix";
var DEFAULT_SHEET_WIDTH = 1e3;
var SHEET_ASPECT_RATIO = DEFAULT_SCHEMATIC_SHEET_WIDTH / DEFAULT_SCHEMATIC_SHEET_HEIGHT;
...
function convertCircuitJsonToStackedSchematicSheetsSvg(circuitJson, options) {
  ...
  const sheetHeight = options?.height ?? Math.round(sheetWidth / SHEET_ASPECT_RATIO);
...
function getSheetLabelPosition({
  ...
  const layout = getSchematicSheetLayout();
  const frameLeft = applyToPoint86(fromString(transformStr), {
    x: layout.minX,
    y: layout.maxY
  });
...
import { parseSync as parseSync4, stringify as stringify7 } from "svgson";
...
import { stringify as stringify8 } from "svgson";
import {
  applyToPoint as applyToPoint87,
...
function createSvgObjectFromPcbBoundary2(transform, minX, minY, maxX, maxY) {
  const [x1, y1] = applyToPoint87(transform, [minX, minY]);
  const [x2, y2] = applyToPoint87(transform, [maxX, maxY]);
  const width = Math.abs(x2 - x1);
  const height = Math.abs(y2 - y1);
  const x = Math.min(x1, x2);
  const y = Math.min(y1, y2);
